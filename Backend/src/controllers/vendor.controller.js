...
export const registerVendor = asyncHandler(async (req, res) => {
  ...
  const {
    storeName, storeDesc, storeEmail, storePhone,
    panNumber, gstNumber, storeAddress,
  } = req.body;
  ...
  res.status(201).json(new ApiResponse(201, { vendor }, 'Vendor registered. Please complete KYC.'));
});
...
export const updateVendorProfile = asyncHandler(async (req, res) => {
  const {
    storeName, storeDesc, storeEmail, storePhone,
    storeAddress, autoAcceptOrders, vacationMode, vacationMessage,
  } = req.body;
  ...
  if (autoAcceptOrders !== undefined) vendor.autoAcceptOrders = autoAcceptOrders;
  if (vacationMode     !== undefined) vendor.vacationMode     = vacationMode;
  if (vacationMessage)   vendor.vacationMessage  = vacationMessage;

  // Handle image uploads
  if (req.files?.logo?.[0]) {
    if (vendor.storeLogo) await UploadService.deleteImage(vendor.storeLogoPublicId).catch(() => {});
    const uploaded   = await UploadService.uploadImage(req.files.logo[0].path, 'vendors/logos');
    vendor.storeLogo = uploaded.url;
  }
  if (req.files?.banner?.[0]) {
    const uploaded     = await UploadService.uploadImage(req.files.banner[0].path, 'vendors/banners');
...
export const submitKYC = asyncHandler(async (req, res) => {
  ...
  if (req.files?.length) {
    for (const file of req.files) {
      const docType = file.fieldname; // 'pan', 'aadhar', 'gst', etc.
      const uploaded = await UploadService.uploadImage(file.path, 'vendors/kyc');
  ...
  await vendor.save();
  res.json(new ApiResponse(200, null, 'KYC submitted. Under review (1-2 business days).'));
});
...
export const updateBankDetails = asyncHandler(async (req, res) => {
  ...
  vendor.bankDetails = {
    ...
    bankName, branchName, isVerified: false,
  };

  await vendor.save();
  res.json(new ApiResponse(200, null, 'Bank details saved. Verification pending.'));
});
...
export const getDashboard = asyncHandler(async (req, res) => {
  const [vendor, summary, orderStats, topProducts] = await Promise.all([
    Vendor.findOne({ user: req.user._id }).select('pendingPayout totalEarnings totalPaidOut totalOrders kycStatus isActive vacationMode'),
...
export const getPayouts = asyncHandler(async (req, res) => {
  ...
  const [payouts, total, vendor] = await Promise.all([
    ...
    Vendor.findOne({ user: req.user._id }).select('pendingPayout totalEarnings totalPaidOut'),
  ]);
  ...
  }, 'Payouts', paginationMeta(total, page, limit)));
});
...
export const requestPayout = asyncHandler(async (req, res) => {
  ...
  if (!vendor.bankDetails?.accountNumber) {
    throw new ApiError(400, 'Please add bank details before requesting a payout');
  }
  ...
  if (payoutAmount < 100) {
    throw new ApiError(400, `Requested amount (₹${payoutAmount}) is less than minimum ₹100`);
  }

  // Check for pending payout already
  const activePayout = await Payout.findOne({ vendor: req.user._id, status: { $in: ['pending', 'processing'] } });
  if (activePayout) {
    throw new ApiError(400, 'A payout is already in progress. Please wait for it to complete.');
  }
  ...
  res.status(201).json(new ApiResponse(201, { payout }, 'Payout request submitted. Admin will process within 3-5 business days.'));
});
...
export const createCoupon = asyncHandler(async (req, res) => {
  const {
    ...
    expiresAt, startsAt, applicableTo, allowedProducts,
  } = req.body;
  ...
  if (applicableTo === 'specific_products' && allowedProducts?.length) {
    ...
    if (ownProducts !== allowedProducts.length) {
      throw new ApiError(403, 'You can only create coupons for your own products');
    }
  }
...
export const getVendorCoupons = asyncHandler(async (req, res) => {
  ...
  res.json(new ApiResponse(200, { coupons }, 'Coupons', paginationMeta(total, page, limit)));
});
...
export const deleteCoupon = asyncHandler(async (req, res) => {
  const coupon = await Coupon.findOne({ _id: req.params.id, createdBy: req.user._id });
  if (!coupon) throw new ApiError(404, 'Coupon not found');
  if (coupon.usedCount > 0) throw new ApiError(400, 'Cannot delete a coupon that has been used');
...
export const getPublicStorePage = asyncHandler(async (req, res) => {
  const vendor = await Vendor.findOne({
    ...
    .select('-bankDetails -documents -panNumber -gstNumber -aadharNumber -kycRejectionReason')
    .lean();
  ...
  res.json(new ApiResponse(200, { vendor, products }, 'Store', paginationMeta(total, page, limit)));
});
...
export const adminGetPendingKYC = asyncHandler(async (req, res) => {
  ...
  const [vendors, total] = await Promise.all([
    Vendor.find({ kycStatus: status })
      ...
      .skip(skip)
      .limit(limit)
      .select('storeName kycStatus documents panNumber gstNumber kycSubmittedAt user')
      .lean(),
    Vendor.countDocuments({ kycStatus: status }),
  ]);

  res.json(new ApiResponse(200, { vendors }, `KYC ${status}`, paginationMeta(total, page, limit)));
});

export const adminReviewKYC = asyncHandler(async (req, res) => {
  const { vendorId } = req.params;
  const { action, reason, documentId, documentStatus, documentRejectionReason } = req.body;
  ...
  if (action === 'approve') {
    ...
    vendor.documents.forEach((d) => { if (d.status === 'pending') d.status = 'approved'; });
...
export const adminProcessPayout = asyncHandler(async (req, res) => {
  ...
  if (payout.status === 'completed') throw new ApiError(400, 'Payout already completed');
