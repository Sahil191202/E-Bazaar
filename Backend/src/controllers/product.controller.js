...
export const createProduct = asyncHandler(async (req, res) => {
  const {
    name, description, shortDesc, categoryId, subCategoryId,
    brand, tags, variants, isFreeShipping, shippingCharge,
  ...
  let images = [];
  if (req.files?.length) {
    const uploaded = await UploadService.uploadMultiple(req.files, `products/${req.user._id}`);
    images = uploaded.map((u, i) => ({ ...u, alt: name, isPrimary: i === 0 }));
  }

  // Parse variants (sent as JSON string in multipart)
  const parsedVariants = typeof variants === 'string' ? JSON.parse(variants) : variants;

  // Validate SKU uniqueness across all variants
  const skus = parsedVariants.map((v) => v.sku);
  if (new Set(skus).size !== skus.length) throw new ApiError(400, 'Duplicate SKUs in variants');

  // Check global SKU uniqueness
  const skuConflict = await Product.findOne({ 'variants.sku': { $in: skus } });
  if (skuConflict) throw new ApiError(409, `SKU already exists: ${skuConflict.variants.find((v) => skus.includes(v.sku))?.sku}`);
  ...
  res.status(201).json(new ApiResponse(201, { product }, 'Product submitted for approval'));
});
...
export const getProducts = asyncHandler(async (req, res) => {
  ...
  if (category) {
    ...
    if (cat) {
      ...
      const descendants = await Category.find({ ancestors: cat._id }).select('_id');
  ...
  if (cached) return res.json(new ApiResponse(200, cached.data, 'Products fetched', cached.meta));
...
export const updateProduct = asyncHandler(async (req, res) => {
  ...
  const { name, description, shortDesc, brand, tags,
          isFreeShipping, shippingCharge, metaTitle, metaDescription } = req.body;
...
export const addVariant = asyncHandler(async (req, res) => {
  ...
  const { sku, price, mrp, stock, attributes, weight, dimensions, lowStockThreshold } = req.body;
  ...
  product.variants.push({ sku, price, mrp, stock, attributes, weight, dimensions, lowStockThreshold, images });
  await product.save();
...
export const updateVariant = asyncHandler(async (req, res) => {
  ...
  const { price, mrp, stock, attributes, weight, dimensions, lowStockThreshold, isActive } = req.body;
...
export const addProductImages = asyncHandler(async (req, res) => {
  ...
  if (product.images.length + req.files.length > 10) throw new ApiError(400, 'Maximum 10 images per product');
...
export const searchSuggestions = asyncHandler(async (req, res) => {
  ...
  const suggestions = {
    products:   products.map((p) => ({ name: p.name, slug: p.slug, image: p.images[0]?.url })),
...
export const getVendorProducts = asyncHandler(async (req, res) => {
  ...
  res.json(new ApiResponse(200, { products }, 'Vendor products', paginationMeta(total, page, limit)));
});
...
export const bulkUploadProducts = asyncHandler(async (req, res) => {
  ...
  if (!rows.length) throw new ApiError(400, 'File is empty or has no valid rows');
  if (rows.length > 500) throw new ApiError(400, 'Maximum 500 products per upload');

  const results = { success: [], failed: [] };

  for (const [index, row] of rows.entries()) {
    try {
      const { name, description, categoryId, sku, price, mrp, stock, brand, tags } = row;

      if (!name || !description || !categoryId || !sku || !price || !mrp) {
        throw new Error('Missing required fields: name, description, categoryId, sku, price, mrp');
      }
      ...
      results.failed.push({ row: index + 2, name: row.name || 'Unknown', error: err.message });
    }
  }
...
export const getInventoryLogs = asyncHandler(async (req, res) => {
  ...
  const product = await Product.findOne({ _id: productId, vendor: req.user._id });
  if (!product) throw new ApiError(404, 'Product not found');
  ...
  res.json(new ApiResponse(200, { logs }, 'Inventory logs', paginationMeta(total, page, limit)));
});
