...
const DEFAULT_COMMISSION = parseFloat(
...
export const initiateOrder = asyncHandler(async (req, res) => {
  ...
  if (walletAmount > 0) {
    if (user.walletBalance < walletAmount) {
      throw new ApiError(
        400,
        `Insufficient wallet balance. Available: ₹${user.walletBalance}`,
      );
    }
  ...
  const orderItems = cartData.items.map((item) => {
    ...
    const { platformEarning, vendorEarning } =
      PaymentService.calculateCommission(itemTotal, DEFAULT_COMMISSION);
    return {
...
export const verifyPayment = asyncHandler(async (req, res) => {
  ...
  if (order.paymentStatus === "paid") {
    return res.json(
      new ApiResponse(200, { orderId: order._id, status: "already_paid" }),
    );
  }
  ...
  if (rzpPayment.status !== "captured") {
    throw new ApiError(
      400,
      `Payment not captured. Status: ${rzpPayment.status}`,
    );
  }

  // Amount in paise verification
  const expectedPaise = Math.round(
    (order.totalAmount - order.walletAmountUsed) * 100,
  );
...
export const getMyOrders = asyncHandler(async (req, res) => {
  ...
  const [orders, total] = await Promise.all([
    Order.find(filter)
      ...
      .limit(limit)
      .select(
        "orderId status paymentStatus totalAmount items deliveryAddress createdAt isCOD estimatedDelivery",
      )
      .lean(),
...
export const cancelOrder = asyncHandler(async (req, res) => {
  ...
  if (order.paymentStatus === "paid" && order.razorpayPaymentId) {
    await initiateRefundInternal(
      order,
      order.totalAmount - order.walletAmountUsed,
      reason,
    );
  }
...
export const requestReturn = asyncHandler(async (req, res) => {
  ...
  if (item.status !== "delivered")
    throw new ApiError(
      400,
      "Item must be delivered before requesting a return",
    );
...
export const getVendorOrders = asyncHandler(async (req, res) => {
  ...
  const [orders, total] = await Promise.all([
    Order.find(filter)
      ...
      .limit(limit)
      .select(
        "orderId status paymentStatus items deliveryAddress createdAt isCOD",
      )
      .lean(),
    Order.countDocuments(filter),
  ]);
...
export const adminGetOrders = asyncHandler(async (req, res) => {
  ...
  const [orders, total] = await Promise.all([
    Order.find(filter)
      ...
      .select(
        "orderId status paymentStatus paymentMethod totalAmount customer createdAt isCOD",
      )
      .lean(),
    Order.countDocuments(filter),
  ]);
...
export const adminInitiateRefund = asyncHandler(async (req, res) => {
  ...
  if (!order.razorpayPaymentId)
    throw new ApiError(400, "No Razorpay payment found for this order");
